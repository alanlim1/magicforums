class CommentBroadcastJob < ApplicationJob
    ...
    def perform(type, comment)
        ActionCable.server.broadcast 'posts_channel', type: type, comment: comment, post: comment.post, email: comment.user.email, partial: render_comment_partial(comment)
    end
